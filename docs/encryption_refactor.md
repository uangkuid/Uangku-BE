# Uangku Zero-Knowledge Encryption — Technical Specification & Refactor Guide

**Status:** Backend implemented (this document reflects the shipped contract, not a proposal)
**Audience:** Backend (Laravel), Client KMP (Android/iOS), Client Web (Vue JS), Security reviewer
**Related:** [`../plan.md`](../plan.md) · [`../handoff.md`](../handoff.md)
...
sequenceDiagram
    ...
    S ->> S: verifikasi OTP
    S ->> S: blind_index = HMAC(email); email_ciphertext = AES-GCM(system_key, email)
...
sequenceDiagram
    ...
    O ->> O: (famPub', famPriv') = keypair baru; re-wrap untuk tiap member tersisa
...
- [ ] **Belum bisa dijalankan di lingkungan ini** (MySQL/Docker tidak tersedia) — jalankan `php artisan test` di
  lingkungan dev yang punya `uangku_test` DB untuk memverifikasi penuh, termasuk test family/wallet yang butuh koneksi
  DB nyata.
